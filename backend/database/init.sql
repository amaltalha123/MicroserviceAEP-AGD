...
BEGIN
    ...
    -- Créer l'équipe (temporairement sans chef)
    ...
    SELECT 
        ...
        v_resolution_token,
        NOW() + INTERVAL '30 days'  -- Token valide 30 jours
    RETURNING id INTO v_team_id;
    
    -- Sélectionner les 3 premiers employés disponibles
    FOR v_employee IN (
        SELECT id, full_name
        FROM employees
        WHERE service_type = p_service_type 
          ...
          AND is_active = true
        ORDER BY 
            total_interventions ASC,    -- Moins d'interventions en priorité
            created_at ASC              -- Plus anciens en priorité (rotation équitable)
        LIMIT 3
    ) LOOP
        v_count := v_count + 1;
        ...
        IF v_count = 1 THEN
            ...
            -- Mettre à jour l'équipe avec le vrai chef
        ...
        INSERT INTO team_members (team_id, employee_id, is_leader)
        VALUES (v_team_id, v_employee.id, v_count = 1);
        
        -- Changer le statut de l'employé à non disponible
        UPDATE employees
    ...
    -- Si moins de 3 employés disponibles, retourner une erreur
    IF v_count < 3 THEN
        RAISE EXCEPTION 'Pas assez d''employés disponibles (% trouvé(s), 3 requis)', v_count;
    END IF;
